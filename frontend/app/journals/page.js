...
function BookCover({ journal }) {
  return <div className="book-cover" style={{ "--cover": journal.cover.background, "--cover-accent": journal.cover.accent }}>
    <span className="cover-issn">ISSN {journal.issn}</span>
    <div className="cover-rule" />
    <span className="cover-title">{journal.name}</span>
    <span className="cover-abbr">{journal.abbr}</span>
...
export default function JournalsPage() {
  ...
  return <div className="shell library-shell">
    ...
    <main className="main library-main">
      <header className="library-hero"><p className="eyebrow">THE JOURNAL LIBRARY</p><h1>期刊书库</h1><p>Paper HOT 追踪的期刊，以可浏览的书架形式呈现。每本书封均对应红榜配置中的期刊名称、ISSN 与出版社来源。</p><div className="library-toolbar"><label className="search"><span className="sr-only">搜索期刊</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索期刊、缩写或出版社" /></label><label className="select-label">出版社<select value={publisher} onChange={(event) => setPublisher(event.target.value)}><option value="all">全部出版社</option>{publishers.map((item) => <option key={item}>{item}</option>)}</select></label><div className="segmented library-priority">{[["all", "全部"], ["core", "核心"], ["watch", "关注"], ["skip", "存档"]].map(([value, label]) => <button key={value} className={priority === value ? "active" : ""} onClick={() => setPriority(value)}>{label}</button>)}</div></div></header>
      <section className="library-heading"><span>共 {visible.length} 本期刊</span><span>点击书封访问出版社</span></section>
      <section className="bookshelf" aria-label="期刊书架">{visible.map((journal) => <a key={journal.abbr} className="book" href={journal.publisherUrl} target="_blank" rel="noreferrer" aria-label={`访问 ${journal.name} 出版社页面`}><BookCover journal={journal} /><span className="book-caption"><b>{journal.abbr}</b><span>{journal.priority === "core" ? "核心追踪" : journal.priority === "watch" ? "关注追踪" : "存档期刊"}</span></span></a>)}</section>

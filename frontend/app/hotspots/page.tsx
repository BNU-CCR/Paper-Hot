...
import { getFeaturedPapers, getHotspots } from "../../lib/data";
...
export default function HotspotsPage() {
  ...
  return <div className="shell"><AppSidebar activePath="/hotspots/" /><main className="main"><section className="hero"><div className="headline"><div><h1>当期热点</h1><p>从近一个月已公开的计算传播论文中识别主要研究议题，并关联可直接阅读的论文。</p></div><div className="stats"><span>{topics.length} 个热点</span><span>{hotspots?.source_paper_count || 0} 篇论文</span></div></div></section>{!topics.length ? <div className="empty-state"><b>当期热点暂未生成</b><span>下一次自动更新会生成新的月度议题。</span></div> : <section className="hotspot-feed"><div className="section-heading"><h2>研究议题</h2><span className="count">{displayPeriod(hotspots?.period_start, hotspots?.period_end)} · 仅基于公开论文</span></div><div className="hotspot-grid">{topics.map((topic) => <article className="hotspot-topic" key={topic.title}><header><h3>{topic.title}</h3><p>{topic.description}</p></header><div className="hotspot-papers">{(topic.paper_ids || []).map((id) => byId.get(Number(id))).filter((paper): paper is Paper => Boolean(paper)).map((paper) => <PaperCard key={paper.id} paper={paper} />)}</div></article>)}</div></section>}</main></div>;
}

...
- 如果改动仅涉及前端（`frontend/` 内的组件、样式、类型或静态展示逻辑），在类型检查和生产构建通过后，Codex 默认直接提交并推送到 `main`，无需新建分支或 PR，也无需再次询问是否推送。
- 如果改动同时涉及后端、数据管线、工作流或其他高风险范围，仍按 `AGENTS.md` 的小 PR 流程处理，除非用户另有明确要求。
- 直接推送前必须确认当前分支为 `main`，且提交中不混入与本次任务无关的改动。
